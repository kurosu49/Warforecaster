...
public class GUI extends JFrame {
	...
	private TournamentPanel myTournamentPanel;
	...
	public void loginSuccess(Strategist aStrategist) {		
		...
		try {
			lblStrategistInfo.setText("Lvl: " + loggedStrategist.getLevel() + 
					", Cash: " + db.getRecentBalance(loggedStrategist.getID()));
		...
		panelCurrent.add(myWelcomePanel);	
	...
	public void updateStrategistStatus() {
		try {
			lblStrategistInfo.setText("Lvl: " + loggedStrategist.getLevel() + 
					", Cash: " + db.getRecentBalance(loggedStrategist.getID()));
		} catch (SQLException e) { e.printStackTrace(); }
	...
	public void panelToGuildhallPanel() {
		myGuildhallPanel = new GuildhallPanel(this, loggedStrategist, db);
	...
	public void panelToTournamentPanel() {
		myTournamentPanel = new TournamentPanel(this, loggedStrategist, db);
		
		if (panelCurrent.getComponentCount() != 0)
			panelCurrent.remove(panelCurrent.getComponent(0));
		panelCurrent.add(myTournamentPanel);
		panelCurrent.revalidate();
		panelCurrent.repaint();
	}
	...
	private void buildGUIMain() {
		...
		btnMenuHome = new JButton("Strategist Info");
		...
		btnMenuTournament.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				panelToTournamentPanel();
			}
		});
